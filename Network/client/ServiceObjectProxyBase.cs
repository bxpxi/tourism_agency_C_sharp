...
namespace Network.client
{
    ...
    {  
        ...
        protected void SendRequest(IRequest request)
        {
            try
            {
                Console.WriteLine($"Sending request: {Stringifier.Encode(request)}");
        ...
        protected abstract void HandleUpdate(UpdatedFlightResponse updatedFlight);
        ...
        public virtual void Run()
        {
            while (!IsFinished) 
            {
                //try
                {
                    ...
                    if (response is UpdatedFlightResponse)
                    {                        
                        HandleUpdate(response as UpdatedFlightResponse);
                    }
                    else
                    {
                        lock (Responses)
